<template>
  <section class="correspondence" aria-label="Ticket conversation">
    <header class="conversation-context">
      <span class="context-avatar" :style="{ background: ticket.avatarColor }">{{ ticket.initials }}</span>
    ...
    <div ref="threadEl" class="correspondence-body" aria-live="polite">
      <article class="correspondence-item is-request">
        <header class="message-header">
          <span class="message-avatar" :style="{ background: ticket.avatarColor }">{{ ticket.initials }}</span>
      ...
      <template v-for="group in groups" :key="group.day">
        ...
        >
          <header class="message-header">
            ...
            >{{ messageInitials(message) }}</span>
            <div class="message-sender">
              <strong>{{ message.authorName }}</strong>
              <span v-if="message.isInternal"><Icon icon="mdi:lock-outline" width="12" height="12" aria-hidden="true" />Internal note <span aria-hidden="true">&middot;</span> Support staff only</span>
          ...
          <div v-if="message.isInternal" class="note-banner"><Icon icon="mdi:lock-outline" width="14" height="14" aria-hidden="true" />This note is not visible to the requester.</div>
